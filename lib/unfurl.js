...
const unfurlProfile = require('./unfurl/profile');
...
module.exports = async (github, url) => {
  const parsedGitHubURL = githubUrl(url);

  if (parsedGitHubURL.type === 'comment') {
    const { owner, repo, number, id } = parsedGitHubURL;
  ...
  } else if (parsedGitHubURL.type === 'issue') {
    const { owner, repo, number } = parsedGitHubURL;
    const issue = await github.issues.get({ owner, repo, number });
  ...
  } else if (parsedGitHubURL.type === 'pull') {
    const { owner, repo, number } = parsedGitHubURL;
  ...
  } else if (parsedGitHubURL.type === 'profile') {
    const { owner } = parsedGitHubURL;
    const user = await github.users.getForUser({ username: owner });
    return unfurlProfile(user.data);
  }
};
