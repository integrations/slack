...
class PullRequest extends AbstractIssue {
  ...
  constructor({
    ...
    if (statuses) {
      this.unsuccessfulStatuses.push(
        ...statuses.filter(status => status.state !== 'success')
      );
    }

    if (checkRuns) {
      this.unsuccessfulStatuses.push(
        ...checkRuns.filter(checkRun => checkRun.conclusion !== 'success')
      );
    }
  }
  ...
  getRenderedMessage() {
    ...
    attachments.push(
      ...this.unsuccessfulStatuses.map(status => new Status(status).renderAttachment())
    )

    if (this.totalNumberOfStatuses > 1) {
      attachments.push(
        // render this message "{n/m} succesful checks"
        Status.getChecksPassAttachment(
