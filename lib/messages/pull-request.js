...
class PullRequest extends AbstractIssue {
  ...
  constructor({
    ...
    if (checkRuns) {
      this.unsuccessfulStatuses.push(...checkRuns.filter(checkRun => checkRun.conclusion !== 'success'));
      this.successfulStatuses.push(...checkRuns.filter(checkRun => checkRun.conclusion === 'success'));
    }
  }
