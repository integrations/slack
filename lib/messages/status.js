...
class Status extends Message {
  constructor(checkOrStatus) {
    super({});
    this.status = checkOrStatus.state || checkOrStatus.conclusion || checkOrStatus.status;
    this.context = checkOrStatus.context || checkOrStatus.name;
    this.description = checkOrStatus.description || checkOrStatus.output.title || '';
  ...
  renderAttachment() {
    ...
    return {
      ...
      color: this.constructor.getStatusColor(this.status),
      mrkdwn_in: ['text'],
    };
  }
}
