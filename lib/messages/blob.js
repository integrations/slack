...
class Blob extends Message {
  ...
  getCore() {
    ...
    // only unfurl binary files that are images
    if (isBinaryFile.sync(buffer, this.blob.size)) {
      const imageExtensions = ['png', 'jpg', 'jpeg', 'gif'];
      const blobExtension = this.blob.name.toLowerCase().split('.')[1];

      if (imageExtensions.includes(blobExtension)) {
        return {
          fallback: `[${this.repository.full_name}] ${title}`,
          title,
          title_link: htmlURL,
          image_url: this.blob.download_url,
        };
