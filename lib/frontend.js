...
app.get('/github/oauth/login', async (req, res) => {
  ...
  if (!await githubSignInAttempt.confirm(teamSlackId, userSlackId)) {
    return res.status(400).send('Error: Either the time window to connect your GitHub account has expired or this sign in was initiated by a different user. Please return to Slack to re-start the process of connecting your GitHub account.');
  }
  ...
  res.render('confirm-login.hbs', {
    ...
    teamSlackDomain,
    continueLink: `https://github.com/login/oauth/authorize?${queryString.stringify(params)}`,
  });
});
