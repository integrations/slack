...
app.get('/github/oauth/login', async (req, res) => {
  ...
  try {
    ...
    if (err.name && err.name === 'JsonWebTokenError') {
      ...
      // Todo: This could be user facing so output a nicer error message here (fix this in testing)
      return res.status(400).send(`Error: ${err.message}`);
    }
    throw err;
  }
  ...
  const githubOAuthState = crypto.randomBytes(Math.ceil(30 / 2)).toString('hex').slice(0, 30);
