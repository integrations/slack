...
const { mergeLabelsAndSettings } = require('../settings-helper');
...
module.exports = async (req, res) => {
  ...
  const settingsData = mergeLabelsAndSettings(parsedSettings);

  if (command.subcommand === 'subscribe') {
    if (subscription) {
      if (parsedSettings.hasValues) {
        req.log.debug( { parsedSettings }, 'Subscription already exists, updating settings. New settings: ');

        subscription.enable(settingsData);
    ...
    if (subscription) {
      if (parsedSettings.hasValues) {
        subscription.disable(settingsData);
