...
module.exports = async (req, res, next) => {
  ...
  if (command.text === '') {
    // When a user types the following incomplete command /github subscribe

    const list = new SubscriptionList(subscriptions, repositories, accounts);
    const subscribedResources = list.getAttachments();

    const response = (new Help(command.command, command.subcommand, subscribedResources)).toJSON();
