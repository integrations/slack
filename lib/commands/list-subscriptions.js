...
module.exports = async (req, res, next) => {
  ...
  if (command.text === 'list') {
    return command.respond(new SubscriptionList(repositories, accounts, command.channel_id));
  }

  const response = (new Help(command.command, command.subcommand)).toJSON();
  const list = new SubscriptionList(repositories, accounts, command.channel_id);
