...
const { labelValuePattern, parseSettings } = require('../settings-helper');
...
module.exports = (sequelize, DataTypes) => {
  ...
  function invalidFeatureValueFailure(key, value) {
    // invalidInputFailure(`\`${rawInput}\` is not a feature.`);
  ...
  Object.assign(Subscription.prototype, {
    ...
    enableFeatures(features) {
      ...
      console.error('welcome to enable features. data: ', features);

      Object.entries(features).forEach(([name, value]) => {
        // for (const [name, value] of Object.entries(features)) {
        // make sure we handle commits: ['all'], correctly
        if (Array.isArray(DefaultSettings[name])) {
          ...
          console.error('non array stuff', name, value);
          console.error(this.settings);
        }
      });
      this.changed('settings', true);
    ...
    enableLabels(labels) {
      ...
      const union = (this.settings.label || []).concat(labels);
      this.settings.label = [...new Set(union)];
      this.changed('settings', true);
    },

    // `Update` existing subscriptions
    enable(input) {
      console.error('enable called by something. Input is: ', input);
      this.enableFeatures(input);
      // this.enableLabels(input.labels);
    },
  ...
  Object.assign(Subscription, {
    ...
    async subscribe(attrs) {
      ...
      // TODO make this nicer and only add the labels key if there are labels...
      // Figure out a reusable way the is not toooo mutating :think:
      // console.error('Welcome to subscribe :)... visitor today: ', attrs);

      if (attrs.settings) {
        // console.error(attrs);
        const { settings, ...rest } = attrs;
        if (settings.invalids.length) {
          const firstError = settings.invalids[0];
          invalidFeatureValueFailure(firstError.key, firstError.val);
        }

        const combinedSettings = Object.assign(settings.features, { label: settings.labels });
        const finalAttrs = Object.assign(attrs, { settings: combinedSettings });

        console.error('in subscribe/create. See our input: ', finalAttrs);
        const record = await this.create(finalAttrs);
