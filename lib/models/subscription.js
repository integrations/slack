...
const { labelValuePattern, reviewerValuePattern, parseFeatures } = require('../settings-helper');
...
module.exports = (sequelize, DataTypes) => {
  ...
  const AllowValues = {
    ...
    required_reviewers: reviewerValuePattern(),
  };
  ...
  // required_labels: ['foo', 'bar']
  // required_reviewers: ['bizz', 'buzz']
  // features: [comments: 'all', issues: boolean]
  // => { features: [...], required_label: [...], required_reviewers: [...]}
  function mergeFiltersAndFeatures(features, required_labels, required_reviewers) {
    if (required_labels.length) { Object.assign(features, { required_labels }); }
    if (required_reviewers.length) { Object.assign(features, { required_reviewers }); }
    return features;
  }
  ...
  Object.assign(Subscription.prototype, {
    ...
    enable(settings) {
      ...
      if (settings) {
        ...
        const mergedSettings = mergeFiltersAndFeatures(
          features,
      ...
      return Object.keys(settings)
        .map((setting) => {
          if (settings[setting] === true) {
            ...
            } else if (setting === 'required_reviewers') {
              const asString = settings[setting][0];
              return `required-reviewers: '${asString}'`;
            } else {
