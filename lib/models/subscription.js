...
module.exports = (sequelize, DataTypes) => {
  ...
  const AllowValues = {
    ...
    ignored_labels: labelValuePattern(),
  };
  ...
  // required_labels: ['foo', 'bar'], ignored_labels ['X'],
  // features: [comments: 'all', issues: boolean]
  // => { features: [...], required_label: [...], ignored_label:[...] }
  function mergeLabelsAndFeatures(required_labels, ignored_labels, features) {
    const merged_required = required_labels.length ?
      Object.assign(features, { required_labels }) : features;
    const merged = ignored_labels.length ?
      Object.assign(merged_required, { ignored_labels }) : merged_required;
    return merged;
  }
    ...
    settings: {
      ...
      validate: {
        validateSetting(settingInput) {
          ...
          if (settingInput.ignored_labels && settingInput.required_labels) {
            if (settingInput.ignored_labels.some(ai => settingInput.required_labels.includes(ai))) {
              invalidInputFailure('Cannot have the same required and ignored labels');
            }
          }
        },
      },
  ...
  Object.assign(Subscription.prototype, {
    ...
    enable(settings) {
      ...
      if (settings) {
        ...
        const mergedSettings = mergeLabelsAndFeatures(settings.required_labels,
          settings.ignored_labels,
          features);
      ...
      return Object.keys(settings)
        .map((setting) => {
          if (settings[setting] === true) {
            ...
            } else if (setting === 'ignored_labels') {
              const asString = settings[setting][0];
              return `ignored-label: '${asString}'`;
            } else {
