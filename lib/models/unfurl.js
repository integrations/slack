...
module.exports = (sequelize, DataTypes) => {
  ...
  Object.assign(Unfurl, {
    ...
    async spawn({
      ...
      // this should be find or create slackUser
      const slackUser = await SlackUser.findOne({
        where: {
      ...
      // If user has not connected, we should still try to unfurl using GITHUB_TOKEN.
      // Need to be able to do that without storing the unfurl as it might fail

      if (await this.isPrivate(github, url)) {
        // check here if this is a link we can even unfurl.
        // no point in creating this if we can't unfurl
        return this.create({
