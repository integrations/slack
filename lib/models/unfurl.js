  ...
  Object.assign(Unfurl, {
    ...
    async promptOrDeliver({
      ...
      if (hasEarlyAccess({ channelId: channel, teamId })) {
        ...
        if (repoIsPrivate) {
          if (await Subscription.lookupOne(repoId, channel, workspace.id) || slackUser.allowsAutomaticUnfurl(repoId)) {
            // No need to prompt because unfurl is part of subscribed repo or user allows automatic unfurl
            isPublic = false;
          } else {
            return this.create({
