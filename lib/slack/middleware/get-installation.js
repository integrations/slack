...
module.exports = async function getInstallation(req, res, next) {
  ...
  try {
    ...
    if (/api\.slack\.com/.test(req.headers['user-agent'])) {
      if (owner.id === parseInt(gitHubUser.id, 10)) {
        return res.json(new InstallGitHubApp({ installLink }));
      }
      return res.json(new InstallGitHubApp({ installLink, ownerType: owner.type }));
    }
