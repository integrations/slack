...
module.exports = async (req, res) => {
  ...
  async function respondWith(message) {
    if (req.accepts('html')) {
      await axios.post(command.response_url, message.toJSON());
  ...
  if (command.subcommand === 'subscribe') {
    if (subscription) {
      if (settings) {
        ...
        respondWith(new Subscribed({
          channelId: to,
          fromRepository: from,
        }));
      }
      req.log.debug('Subscription already exists');
      return respondWith(new AlreadySubscribed(req.body.args[0]));
    }
    ...
    return respondWith(new NotSubscribed(req.body.args[0]));
  }
};
