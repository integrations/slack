...
// @todo should also be able to unlink (/github unlink)
module.exports = async (command, subscriptionRequest = null) => {
  ...
  const signupLink =
    `https://${process.env.APP_URL}/github/oauth/login?state=${escapedEncryptedState}`;
  return {
    response_type: 'ephemeral',
    attachments: [{
      text: `<${signupLink}|Finish linking your account>`,
    }],
  };
};
