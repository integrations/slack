...
module.exports = {
  async login(req, res) {
    ...
    res.redirect(`${slackRootUrl}/oauth/v2/authorize?client_id=${clientId}&scope=${scope}&state=${state}`);
  },
  ...
  async callback(req, res) {
    ...
    try {
      ...
      access = await client.oauth.v2.access({
        client_id: process.env.SLACK_CLIENT_ID,
        client_secret: process.env.SLACK_CLIENT_SECRET,
        code: req.query.code,
      });
    } catch (e) {
      req.log.debug(e, 'Failed to exchange code for access token');
      return res.redirect('/slack/oauth/login');
    }

    req.log.debug(access, 'Exchanged code for access token');

    const installingEnterpriseId = access.enterprise && access.enterprise.id;

    if (process.env.SLACK_ENTERPRISE_ID
      && installingEnterpriseId !== process.env.SLACK_ENTERPRISE_ID) {
      req.log.info(`Workspace (${access.team.name} - ${access.team.id}) not in enterprise tried to install`);
      return res.redirect('/slack/oauth/login');
    }

    const [workspace, created] = await SlackWorkspace.findOrCreate({
      where: { slackId: access.team.id },
    ...
    // Post on-boarding message to installing user
    await workspace.client.chat.postMessage({
      channel: access.authed_user.id,
      ...new SlackInstalled().toJSON(),
    });

    return res.redirect(`${slackRootUrl}/app_redirect?app=${access.app_id}&team=${access.team.id}`);
  },
};
