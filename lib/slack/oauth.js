...
module.exports = {
  ...
  async callback(req, res) {
    ...
    if (access.ok) {
      // Check that the teams are allowed to use this app
      if (process.env.ALLOWED_TEAMS) {
        const ws = slack.createClient(access.access_token);
        const allowedTeams = process.env.ALLOWED_TEAMS.split(',');

        const info = await ws.team.info();
        req.log.debug(info.team, 'Looked up slack team');

        if (!allowedTeams.includes(info.team.domain)) {
          req.log.info(info.team, 'Slack team not allowed');
          await ws.auth.revoke();
