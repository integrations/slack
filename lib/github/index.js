...
module.exports = (robot) => {
  ...
  app.get('/setup', async (req, res) => {
    const { SlackUser, SlackWorkspace, PendingCommand } = robot.models;

    let user;

    if (req.session && req.session.slackUserId) {
      user = await SlackUser.findById(req.session.slackUserId, { include: [SlackWorkspace] });
    }

    if (user && await PendingCommand.find(user.SlackWorkspace.slackId, user.slackId)) {
