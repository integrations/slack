  ...
  robot.route().get('/github/oauth/callback', async (req, res) => {
    ...
    if (state.subscriptionRequest) {
      ...
      const response = subscribe({
        ...
        text: state.subscriptionRequest,
      }, { robot, router });
      robot.log.trace(slackResponseUrl, response, 'User authenticated, finish subscription request');
      await axios.post(slackResponseUrl, response);
    }
  });
};
