...
module.exports = (robot) => {
  ...
  robot.route().get('/github/oauth/callback', async (req, res) => {
    ...
    const { SlackUser, GitHubUser } = robot.models;
    ...
    robot.log({ created, gitHubUser }, 'Linked GitHubUser');

    const slackUser = await SlackUser.findOne({ where: { slackId: slackUserId } });

    if (slackUser) {
      await slackUser.update({ githubId: gitHubUser.id });
    } else {
      await SlackUser.create({ slackId: slackUserId, githubId: gitHubUser.id });
    }
