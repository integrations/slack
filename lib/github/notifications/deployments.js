...
async function deploymentStatus(context, subscription, slack) {
  ...
  if (storedMetaData) {
    const res = await slack.chat.update(storedMetaData.ts, storedMetaData.channel, '', deploymentStatusMessage.getRenderedMessage());
    context.log(res, 'Updated Slack message');
  } else {
    const res = await slack.chat.postMessage(subscription.channelId, '', deploymentStatusMessage.getRenderedMessage());
