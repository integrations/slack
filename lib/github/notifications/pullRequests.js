...
async function pullRequestOpened(context) {
  ...
  slack.web.chat.postMessage('#general', '', prBase, // 2nd argument is required in API wrapper, but not in API (We don't want to use it in this case)
    (err, res) => {
      if (err) {
...
async function onStatus(context) {
  ...
  statuses.data.statuses.forEach((status) => {
    prBase.attachments.push({
      ...
      author_name: status.context,
      author_icon: status.avatar_url,
      author_link: status.target_url,
  ...
  slack.web.chat.update(message.ts, message.channel, '', prBase,
    (err, res) => {
      if (err) {
        console.log('Error:', err);
      } else {
