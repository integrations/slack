...
const { PullRequest } = require('./renderer');
...
async function pullRequestOpened(context) {
  ...
  slack.chat.postMessage('#general', '', prMessage.getRenderedMessage(), (err, res) => {
    if (err) {
...
async function onStatus(context) {
  ...
  // 2nd argument is required in API wrapper, but not in API (We don't want to use it in this case)
  slack.chat.update(message.ts, message.channel, '', prMessage.getRenderedMessage(), (err, res) => {
    if (err) {
      console.log('Error:', err);
    } else {
      console.log('Message sent: ', res);
