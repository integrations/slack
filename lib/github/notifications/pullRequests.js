...
async function pullRequestEvent(context, subscription, slack) {
  ...
  if (eventType === 'pull_request.opened') {
    cache.set(`pull-${context.payload.pull_request.id}`, messageMetaData); // store PR id so that the PR can be mapped to the slack message
  }
}

function storePRMapping(context) {
  cache.set(`status-${context.payload.pull_request.head.sha}`, context.payload.pull_request.id); // store sha so that status events can be linked back to PR
}
...
async function onStatus(context, subscription, slack) {
  ...
  const message = await cache.get(`pull-${id}`);
