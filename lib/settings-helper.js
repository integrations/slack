...
function parseSettings(commandInput) {
  ...
  const parsed = commandArgs.reduce((result, arg) => {
    ...
    if (arg.startsWith('-label')) {
      ...
      const value = extractLabelValue(arg);
      return Object.assign(result, { ignored_labels: result.ignored_labels.concat(value) });
    }
  ...
  return {
    ...
    ignored_labels: uniq(parsed.ignored_labels),
    ...
    hasValues: [parsed.features, parsed.required_labels, parsed.ignored_labels, parsed.invalids]
