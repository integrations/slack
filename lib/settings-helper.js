...
function parseSettings(commandInput) {
  ...
  return {
    ...
    // NOTE: hasValue needs to be true even for invalid input, to make sure
    // unsubscribe with invalid values does not remove the entire subscription
    hasValues: [parsed.features, parsed.labels, parsed.invalids].some(arr => arr.length > 0),
  };
}
