...
async function submit(req, res) {
  ...
  const response = await slackUser.GitHubUser.client.repos.createDeployment({
    owner,
    repo,
    ref,
  ...
  await slack.chat.postMessage({
    ...
    text: `Deployment of \`${deployment.ref}\` to environment \`${deployment.environment}\` for \`${owner}/${repo}\` was created.`,
  });
