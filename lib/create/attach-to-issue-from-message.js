...
async function createCommentDialogSubmission(req, res) {
  ...
  if (url) {
    ...
    if (!(await Installation.assertPermissions(await robot.auth(), { owner, repo, permissions }))) {
      ...
      return res.send({
        errors: [{
          name: 'url',
          error: `Insufficient permissions. Please make sure the Slack app is installed on this repository: ${appUrl}`,
        }],
      });
    }
  }
  ...
  try {
    ...
    if (!(err.name && err.name === 'GraphQLError')) {
      throw err;
    }
    ...
    return res.send({
      errors: [{
        name: 'issueId',
        error: `Please make sure the Slack app has been granted issue and pull request write access on this repository: ${appUrl}`,
      }],
    });
  }
