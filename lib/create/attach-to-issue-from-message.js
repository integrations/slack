...
async function createCommentDialogSubmission(req, res) {
  ...
  // TODO: Should check here whether we have write access on that repo for issues and if not return an error
  ...
  try {
    ...
    await axios.post(response_url, new DialogSubmissionError('Could not create comment', submission, appUrl).toJSON());
    return res.send();
  }
