...
async function openDialog(req, res) {
  ...
  if (subscriptions.length === 0) {
    return command.respond('This channel is not subscribed to any repositories');
  }
...
async function dialogSubmit(req, res) {
  const slackWorkspace = SlackWorkspace.findOne({ where: { slackId: req.body.team.id } });
  const slackUser = SlackUser.findOrCreate({
    where: { slackId: req.body.user.id, slackWorkspaceId: slackWorkspace.id },
    include: [GitHubUser],
  });

  if (req.body.callback_id === 'new-issue-dialog') {
    // make GitHub API call to create issue
