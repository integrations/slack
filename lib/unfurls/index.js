      ...
      if (err.name === 'GitHubAccountNotConnected') {
        ...
        try {
          ...
          if (error.data && error.data.error === 'channel_not_found') {
            const stateWithMofidiedChannel = new SignedParams({
              slackEvent: {
                ...req.body,
                token: undefined,
                event: {
            ...
            }, 'Could not send ephemeral unfurl signin prompt in channel. Re-trying in direct message');
            await workspace.client.chat.postEphemeral({
              channel: event.user,
              user: event.user,
              attachments: [new PromptSignIn(`${protocol}://${host}/github/oauth/login?state=${await stateWithMofidiedChannel.stringify()}`).getAttachment()],
            });
          } else {
            throw error;
          }
