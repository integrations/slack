...
async function unfurl(github, url, unfurlType) {
  ...
  return {
    message: await resources[params.type](params, github, unfurlType),
...
async function handler(req, res) {
  ...
  const { SlackWorkspace, Unfurl } = robot.models;
  ...
  await Promise.all(event.links.map(async (link) => {
    if (await isEligibleForUnfurl(req.body.team_id, event.channel, link.url)) {
      ...
      try {
        const { message, githubType } = await unfurl(github, link.url, type);
        unfurls[link.url] = message;
        unfurlsForDB.push({
          githubType,
  ...
  if (Object.keys(unfurls).length !== 0) {
    ...
    const slackRes = await workspace.client.chat.unfurl(event.message_ts, event.channel, unfurls);
    await Promise.all(unfurlsForDB.map(unfurlForDB => Unfurl.log(
      workspace.id,
      event.user,
      event.channel,
      unfurlForDB.githubType,
      unfurlForDB.url,
      unfurlForDB.condensed,
      true, // public unfurl
    )));
