...
async function onStatusOrCheckRun(context, subscription, slack) {
  ...
  if (context.event === 'check_run') {
    sha = context.payload.check_run.check_suite.head_sha;
    id = await cache.get(`repo#${context.payload.repository.id}:status#${sha}`); // 287086736
  }

  const cacheKey = subscription.cacheKey(`pull#${id}`); // channel#1#CJPLMMMJQ:pull#287086736
  const message = await cache.get(cacheKey);

  if (!message) {
    context.log.debug({ sha: sha }, `Could not find pull request for ${context.event}`);
    return;
  }
