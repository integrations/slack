...
module.exports = function route(callback) {
  return async (context) => {
    if (context.payload.repository) {
      ...
      // console.error(`processing ${subscriptions.length} subs for ${eventType}`);
      ...
      const promise = Promise.all(subscriptions.map(async (subscription) => {
        ...
        if (subscription.creatorId && eventType !== 'repository.deleted') {
          ...
          const hasRepoAccess = await cache.fetch(
            cacheKey,
            () => {
              console.error(`going to check repo access for event: ${eventType} and ${subscription.channelId}.`);
              return creator.GitHubUser.hasRepoAccess(context.payload.repository.id);
            },
            10 * 60 * 1000,
          );
        ...
        const githubName = subscription.type === 'account'
