...
const { shouldFilterByRequiredLabel, shouldFilterByIgnoredLabel } = require('./filters');
  ...
  return async (context) => {
    if (context.payload.repository) {
      ...
      const promise = Promise.all(subscriptions.map(async (subscription) => {
        ...
        if (shouldFilterByRequiredLabel(subscription, issue)) {
          const labelNames = issue.labels.map(label => label.name);
          const requiredList = subscription.settings.required_labels;

          if (!labelNames.some(l => requiredList.includes(l))) {
            context.log({ labelNames, requiredlist: requiredList, eventType }, 'Discarding event. Required labels not found.');
            return;
          }
        }

        if (shouldFilterByIgnoredLabel(subscription, issue)) {
          const labelNames = issue.labels.map(label => label.name);
          const ignoredList = subscription.settings.ignored_labels;

          if (labelNames.some(l => ignoredList.includes(l))) {
            context.log({ labelNames, ignoredlist: ignoredList, eventType }, 'Discarding event. Ignored label found.');
            return;
          }
        }
