...
const { shouldFilterByLabel } = require('./filters');
      ...
      const promise = Promise.all(subscriptions.map(async (subscription) => {
        ...
        if (shouldFilterByLabel(subscription, issue)) {
          ...
          if (!labels.some(l => whitelist.includes(l))) {
            const message = `Discarding event ${eventType}. Required labels not found.`;
            context.log({ issueLabel: labels, whiteList: whitelist }, message);
            return;
          }
        }
