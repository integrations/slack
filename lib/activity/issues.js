...
async function updateIssueMessage(messageMetaData, context, subscription, slackWorkspace) {
...
async function matchMetaDataStatetoIssueMessage(context, subscription, slackWorkspace) {
  const cacheKey = subscription.cacheKey(`issue#${context.payload.issue.id}`);
  const storedMetaData = await cache.get(cacheKey);
  if (storedMetaData) {
    await updateIssueMessage(storedMetaData, context, subscription, slackWorkspace);
  }
}

async function issueEvent(context, subscription, slackWorkspace) {
  let issue = {
  ...
  if (eventType === 'issues.opened') {
    ...
    await matchMetaDataStatetoIssueMessage(context, subscription, slackWorkspace);
  }
}
